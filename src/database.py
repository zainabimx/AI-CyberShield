...
def delete_alert(alert_id):
    ...
    try:
        ...
        conn.execute("DELETE FROM incident_reports WHERE alert_id = ?", (str(alert_id),))
    ...
    try:
        ...
        for row in rows:
            try:
                ...
                continue
        if ids:
            conn.executemany("DELETE FROM analyst_notes WHERE alert_id = ?", [(x,) for x in ids])
            conn.executemany("DELETE FROM incident_reports WHERE alert_id = ?", [(x,) for x in ids])
